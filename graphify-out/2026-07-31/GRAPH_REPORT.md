...
## Corpus Check
- 112 files · ~394,595 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 952 nodes · 1918 edges · 65 communities (56 shown, 9 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 209 edges (avg confidence: 0.53)
...
1. `err()` - 64 edges
2. `VpnManager()` - 35 edges
3. `tr()` - 22 edges
4. `RouteManager()` - 21 edges
5. `parseProxyLink()` - 21 edges
6. `wire()` - 20 edges
...
- `copyFileAtomic()` --calls--> `ensureDir()`  [EXTRACTED]
  app/service/lib/core-integrity.js → app/service/lib/atomic.js
...
Cohesion: 0.08
Nodes (71): applyBootstrap(), build(), canonicalHost(), endpoints(), outboundFor(), own(), tlsFor(), transportFor() (+63 more)

### Community 2 - "err"
Cohesion: 0.18
Nodes (10): Alcyone maintainer audit - final state, Canonical unpublished packages, Changes made, Complete test results, Executive result, Final canonical-package attempt, Original maintainer items, Physical evidence (+2 more)
...
Cohesion: 0.19
Nodes (4): generateCode(), generateToken(), PairingManager(), safeEqual()
...
Cohesion: 0.12
Nodes (19): guard(), copy(), load(), merge(), paths(), isAlcyoneError(), toResult(), callerAllowed() (+11 more)
...
Cohesion: 0.13
Nodes (7): Alcyone application source, Release history, Alcyone 3.0.1, Alcyone 3.0.2, Alcyone 3.2.1, Alcyone 4.0.0, Alcyone 4.0.3
...
Cohesion: 0.13
Nodes (12): Api(), accessMode(), cacheKey(), canAccess(), copy(), probe(), readAppPayloadReadable(), readDataDirWritable() (+4 more)
...
Cohesion: 0.06
Nodes (63): copyFileAtomic(), expectedFor(), matches(), prepare(), regularFile(), sha256File(), verifyForLaunch(), verifyPackaged() (+55 more)
...
Cohesion: 0.19
Nodes (3): fileExists(), Migrator(), readPid()

### Community 53 - "xray.js"
Cohesion: 0.05
Nodes (42): applyBootstrap(), applyResourcePolicy(), applyXhttpLimits(), boundedPolicyValue(), build(), buildFullConfig(), buildStreamSettings(), canonicalHost() (+34 more)
...
## Knowledge Gaps
- **135 isolated node(s):** `name`, `version`, `description`, `main`, `private` (+130 more)
...
- **Why does `err()` connect `http-client.test.js` to `parsers.js`, `service.js`, `PairingManager`, `atomic.js`, `ImporterServer`, `ProfileStore`, `RouteManager`, `privilege.js`, `logger.js`, `xray.js`, `DeviceInfo`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `register()` connect `RouteManager` to `http-client.test.js`, `app.js`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `VpnManager()` connect `service.js` to `errors.js`, `RouteManager`, `xray.js`?**
