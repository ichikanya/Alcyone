...
var editionLib = require(
  path.join(ROOT, "app", "service", "lib", "edition.js"),
);
...
function testDns(next) {
  ...
  dnsResolver.resolveAll("node012.test", function (error, addresses) {
    ...
    dns.resolve6 = r6;
    record(
      "Node 0.12-compatible DNS returns address objects",
      !error &&
        addresses.length === 1 &&
...
function testNestedFailure(next) {
  ...
  httpClient.fetchUrl = function (url, options, cb) {
    ...
    setImmediate(function () {
      if (url.indexOf("/1") >= 0) cb(new Error("failed"));
        ...
        cb(
          null,
          "dmxlc3M6Ly8xMTExMTExMS0yMjIyLTMzMzMtNDQ0NC01NTU1NTU1NTU1NTV9hLmV4YW1wbGU6NDQz",
        );
    });
  };
  subscriptions.expandNested(
    ...
    function (error) {
      ...
      record(
        "nested subscription failure completes instead of deadlocking",
...
function testAutostart(next) {
  ...
  record(
    "legacy autostart hook is not reported as current",
    autostart.isEnabled() === false,
  );
  record(
    "legacy enabled hook is repaired",
    autostart.repairLegacy() === true && autostart.isEnabled(),
  );
  record(
    "autostart identifies the app on Luna",
...
function testLunaAuthorization(next) {
  ...
  record(
    "own TV app is accepted as Luna caller",
    serviceLib.callerAllowed({ sender: "com.alcyone.vpn" }) === true,
  );
  record(
    "another installed app is rejected as Luna caller",
    serviceLib.callerAllowed({ sender: "com.attacker.app" }) === false,
  );
  record(
    "payload cannot spoof the Luna sender",
    serviceLib.callerAllowed({
      sender: "",
      payload: { $sender: "com.alcyone.vpn" },
    }) === false,
  );
  record(
    "registered Luna method rejects before invoking the VPN API",
    rejected &&
      rejected.returnValue === false &&
      rejected.errorCode === "UNAUTHORIZED",
  );
  next();
}
...
function testEditionAndEarlyLogs(next) {
  ...
  record(
    "service loads generated sing-box identity from its own directory",
    loaded.id === "sing-box" &&
  ...
  var earlyLogger = new loggerLib.Logger({
    file: path.join(blockedParent, "service.log"),
  });
  earlyLogger.error("early startup marker");
  record(
    "startup diagnostics remain readable when the log directory is unavailable",
...
function testLegacyRoutes(next) {
  ...
  record(
    "legacy route state is converted before old PID files are removed",
    converted &&
      state.original.gateway === "192.168.1.1" &&
      state.original.device === "wlan0" &&
      state.serverAddresses.length === 2,
  );
  next();
}
    ...
    importer.listen(true, function (lanError) {
      ...
      setTimeout(function () {
        var lanOk =
          !lanError &&
          callbacks === 1 &&
          importer.boundHost === "0.0.0.0" &&
          importer.server &&
          importer.server.address();
        record(
          "loopback-to-LAN switch has one callback and one tracked server",
          lanOk,
...
function testLogInodeStability() {
  ...
  for (i = 0; i < 9000; i++)
    big += "core line " + i + " padding padding padding\n";
  fs.writeFileSync(capped, big);
  before = fs.statSync(capped).ino;
  record(
    "capFile trims a log past the cap",
    loggerLib.capFile(capped) === true,
  );
  after = fs.statSync(capped).ino;
  record("capFile keeps the same inode", before === after);
  record(
    "capFile actually shrank the file",
    fs.statSync(capped).size < big.length,
  );
  ...
  record(
    "core output after a cap still reaches the log",
  ...
  fs.closeSync(childFd);
  record(
    "core output after clear() still reaches the log",
    logger.tail(50).indexOf("POST-CLEAR") >= 0,
  );

  record(
    "clear() on a missing file is not an error",
    new loggerLib.Logger({ file: path.join(dir, "absent.log") }).clear() ===
      true,
  );
  ...
  var serviceRunLogger = new loggerLib.Logger({
    file: path.join(dir, "run.log"),
    runId: "abcdef123456",
    source: "service",
  });
  var coreRunLogger = new loggerLib.Logger({
    file: path.join(dir, "core-run.log"),
    runId: serviceRunLogger.runId,
    source: "core",
  });
  serviceRunLogger.info("current service event");
  coreRunLogger.info("current core event");
  record(
    "service and core logs share a validated run id",
    serviceRunLogger.runId === "abcdef123456" &&
      coreRunLogger.runId === serviceRunLogger.runId,
  );
  record(
    "log lines identify source and current run",
    /run=abcdef123456 source=service current service event/.test(
      serviceRunLogger.tail(10),
    ) &&
      /run=abcdef123456 source=core current core event/.test(
        coreRunLogger.tail(10),
      ),
  );
  record(
    "fresh logger run ids are ephemeral and bounded",
...
function testImporterCsp() {
  ...
  record(
    "CSP allows same-origin XHR from the importer page",
    csp.indexOf("connect-src 'self'") >= 0,
    csp,
  );
  record(
    "CSP still denies everything by default",
    csp.indexOf("default-src 'none'") >= 0,
  );
  record(
    "CSP grants no wildcard connect source",
    csp.indexOf("connect-src *") < 0,
  );
}
...
function testResidency() {
  ...
  var removed = null;
  if (had)
    removed = process.argv.splice(
      process.argv.indexOf("--disable-timeouts"),
      1,
    );
  serviceLib.keepResident();
  record(
    "service marks itself long-running before the bus is constructed",
    process.argv.indexOf("--disable-timeouts") !== -1,
  );
  serviceLib.keepResident();
  record(
    "residency flag is not duplicated on repeat calls",
    process.argv.filter(function (a) {
      return a === "--disable-timeouts";
    }).length === 1,
  );
  if (!had && removed === null) {
...
function testOverlappingListen(next) {
  ...
  function finish(timedOut) {
    ...
    clearTimeout(guard);
    record(
      "overlapping loopback and LAN binds both succeed",
      !timedOut && !loopbackError && !lanError,
      timedOut
        ? "timed out: " + done + "/2 callbacks"
        : (loopbackError || lanError || {}).code || "",
    );
    record(
      "importer ends up on the last requested scope",
      importer.boundHost === "0.0.0.0",
  ...
  importer.listen(false, function (e) {
    loopbackError = e;
    both();
  });
  importer.pairing.enable();
  importer.listen(true, function (e) {
    lanError = e;
    both();
  });
}
...
function testPlatformEntryPoint() {
  var pkg = JSON.parse(
    fs.readFileSync(path.join(ROOT, "app", "service", "package.json"), "utf8"),
  );
  record(
    "package.json main resolves to the service entry file",
    pkg.main === "service.js",
  );

  record(
    "service exports no run(): bootstrap-node would call it with an " +
      "undeclared `name` and throw",
    serviceLib.run === undefined,
  );

  record(
    "service exposes the launcher check it starts from",
    typeof serviceLib.launchedByPlatform === "function",
  );

  /* Requiring the module from a test must not start a service. */
  record(
    "requiring the service from a test does not self-start",
  ...
  try {
    process.mainModule = {
      filename: "/usr/palm/services/jsservicelauncher/bootstrap-node.js",
    };
    record(
      "the platform launcher is recognized as a start trigger",
  ...
  record(
    'uncaught exceptions record a reason, not just "error"',
