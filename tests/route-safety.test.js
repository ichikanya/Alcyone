...
manager.ip = function (args) {
  ...
  if (args[0] === 'route' && args[1] === 'get') {
    return { code: 0, stdout: String(args[2]) + ' via 192.168.1.1 dev wlan0\n' };
  }
  if (args[0] === 'route' && args[1] === 'show' && args[2] === 'exact' &&
      args[3] === '0.0.0.0/8') {
    return { code: 0, stdout: '0.0.0.0/8 via 192.168.1.1 dev wlan0\n' };
  }
  return { code: 0, stdout: '' };
};
manager.available = function () { return true; };
manager.readDefaultRoute = function () {
  return { gateway: '192.168.1.1', device: 'wlan0', raw: 'default via 192.168.1.1 dev wlan0' };
};
...
record('existing server route is snapshotted',
  state.serverRoutes['203.0.113.10'].indexOf('192.168.1.254') >= 0);
record('existing IPv6 routes are snapshotted',
  !!state.ipv6Routes['::/1'] && !!state.ipv6Routes['8000::/1']);
record('existing direct-range route is snapshotted',
  state.directRoutes['10.0.0.0/8'].indexOf('dev eth0') >= 0);

commands = [];
manager.applyTunRoutes(state);
record('IPv6 is blocked while the IPv4-only VPN is active',
  hasCommand(['-6', 'route', 'replace', 'unreachable', '::/1', 'metric', '42760']) &&
  hasCommand(['-6', 'route', 'replace', 'unreachable', '8000::/1', 'metric', '42760']));
record('only resolved proxy endpoints receive bypass routes, never public DNS',
  hasCommand(['route', 'replace', '203.0.113.10', 'via', '192.168.1.1', 'dev', 'wlan0']) &&
  !commands.some(function (command) {
    var text = command.join(' ');
    return text.indexOf('1.1.1.1') >= 0 || text.indexOf('8.8.8.8') >= 0 ||
      text.indexOf('9.9.9.9 via') >= 0;
  }));
record('XRay direct IPv4 ranges bypass tun0 on the physical interface',
  hasCommand(['route', 'replace', '10.0.0.0/8', 'via', '192.168.1.1', 'dev', 'wlan0']) &&
...
record('every direct-range route is verified off tun0 after split-route install',
...
commands = [];
manager.rollback();
record('pre-existing server route is restored',
  hasCommand(['route', 'replace', '203.0.113.10', 'via', '192.168.1.254', 'dev', 'wlan0', 'metric', '50']));
record('pre-existing IPv6 route is restored',
  hasCommand(['-6', 'route', 'replace', '::/1', 'via', '2001:db8::1', 'dev', 'eth0', 'metric', '100']));
record('pre-existing direct-range route is restored',
  hasCommand(['route', 'replace', '10.0.0.0/8', 'via', '192.168.1.254', 'dev', 'eth0', 'metric', '60']));

manager.readDefaultRoute = function () { return null; };
var missingDefaultCode = '';
try { manager.saveState([]); } catch (missingDefaultError) { missingDefaultCode = missingDefaultError.code; }
record('route snapshot fails before core startup when no physical default is available',
  missingDefaultCode === 'ROUTE_FAILED');

var procRoute = path.join(dir, 'proc-route');
fs.writeFileSync(procRoute,
...
fallbackManager.ip = function () { return { code: -1, stdout: '', stderr: 'transient spawn failure' }; };
var fallbackDefault = fallbackManager.readDefaultRoute();
record('kernel route table preserves the physical default when ip spawn is transiently unavailable',
  fallbackDefault && fallbackDefault.device === 'wlan0' &&
  fallbackDefault.gateway === '192.168.50.1');

manager.readDefaultRoute = function () { return { device: 'eth0', gateway: '192.168.1.1' }; };
record('network-change detection notices a physical interface transition',
  manager.networkChanged(state) === true);
manager.readDefaultRoute = function () { return { device: 'wlan0', gateway: '192.168.1.1' }; };
record('network-change detection accepts the captured physical route',
  manager.networkChanged(state) === false);

var changedNetworkManager = new routesLib.RouteManager({
  core: 'xray', stateFile: path.join(dir, 'changed.state'), ipBinary: '/sbin/ip'
});
var changedCommands = [];
...
changedNetworkManager.available = function () { return true; };
...
record('network-change rollback does not restore stale gateway or interface routes',
