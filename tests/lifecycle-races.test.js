...
async function supervisorIdentityTests() {
  ...
  var supervisor = new supervisorLib.Supervisor({
    stopGraceMs: 5,
    onExit: function (name, code, signal) {
      exits.push([name, code, signal]);
    },
  });
  var ignoredOptions;
  childProcess.spawn = function () {
    if (arguments[2] && arguments[2].stdio === "ignore")
      ignoredOptions = arguments[2];
  ...
  try {
    ...
    check(
      "late exit from child A cannot affect child B",
      supervisor.entryFor("xray") === b && !b.exited && exits.length === 1,
    );

    children[0].emit("error", { code: "ENOENT" });
    check(
      "late spawn error from child A cannot affect child B",
      !b.exited && exits.length === 1,
    );

    var stopDone = false;
    supervisor.stop("xray", function () {
      stopDone = true;
    });
    await later(15);
    ...
    check(
      ...
      supervisor.count() === 1 && supervisor.entryFor("xray") === c,
    );
    ...
    check(
      "supervisor preserves the native single ignore stdio mode",
      ignoredChild && ignoredOptions && ignoredOptions.stdio === "ignore",
    );
  } finally {
...
function makeManager(options) {
  ...
  store.upsertManualProfile(
    "vless://11111111-2222-3333-4444-555555555555@edge.example.com:443?security=tls&type=ws#Edge",
    "Edge",
  );
  var manager = new vpnLib.VpnManager({
    ...
    lock: new lockLib.TunnelLock({
      edition: "xray",
      lockFile: path.join(dir, "tunnel.lock"),
    }),
    coreIntegrity: {
    ...
    dataPlaneProbe: function (callback) {
      if (options.dataPlaneHang) return;
  ...
  manager.routes.saveState = function () {
    metrics.save++;
    this.state = state;
    return state;
  };
  ...
  manager.spawnLogged = function (name) {
    var entry = {
      name: name,
      generation: ++metrics.spawn,
      pid: 8000 + metrics.spawn,
      exited: false,
    };
  ...
  manager.supervisor.stopAll = function (callback) {
    ...
    for (name in this.children)
      if (Object.prototype.hasOwnProperty.call(this.children, name))
        this.children[name].exited = true;
    this.children = {};
  ...
  manager.socksReady = function () {
    metrics.socks++;
    return options.socksReady !== false;
  };
  ...
  return {
    ...
    resolve: function (index) {
      resolvers[index || 0](null, bootstrap);
    },
  };
}
...
async function managerRaceTests() {
  ...
  ctx.manager.connect(function (error) {
    connectCalls++;
    check(
      "disconnect during endpoint resolution cancels connect",
  ...
  var disconnectPromise = new Promise(function (resolve) {
    ctx.manager.disconnect(function (error) {
      disconnectCalls++;
      check("disconnect during endpoint resolution completes", !error);
      resolve();
    });
  });
  ...
  await later(10);
  check(
    "endpoint cancellation leaves no config, routes, spawn or active operation",
    ctx.metrics.config === 0 &&
      ctx.metrics.save === 0 &&
      ctx.metrics.routes === 0 &&
      ctx.metrics.spawn === 0 &&
      ctx.manager.activeOperation === null &&
  ...
  ctx.manager.connect(function (error) {
    readinessCalls++;
    check(
      "disconnect during readiness cancels connect once",
      !!error && error.code === "CANCELLED",
    );
  });
  ctx.resolve(0);
  ...
  await later(300);
  check(
    "readiness cancellation has no stale waiter callback or supervisor entries",
    disconnected &&
      readinessCalls === 1 &&
      ctx.manager.activeOperation === null &&
      ctx.manager.supervisor.count() === 0,
  );
  ...
  check(
    "one transient pre-readiness XRay exit is recovered without duplicate cores",
    !retryError &&
      ctx.manager.status().connected &&
      ctx.metrics.spawn === 3 &&
      ctx.manager.supervisor.count() === 2,
  );
  await new Promise(function (resolve) {
    ctx.manager.disconnect(function () {
      resolve();
    });
  });
  ...
  ctx.manager.connect(function (error) {
    newCallbackCount++;
    check("new connect remains independent of stale callback", !error);
  });
  ctx.resolvers[0](null, ctx.bootstrap);
  ctx.resolve(1);
  await later(10);
  check(
    "stale endpoint callback cannot start a new generation",
    oldCallbackCount === 1 && newCallbackCount === 1 && ctx.metrics.spawn === 2,
  );

  ctx = makeManager();
  var aliveCalls = 0,
    finalCalls = 0;
  ctx.manager.coresAlive = function () {
    aliveCalls++;
    return aliveCalls < 4;
  };
  ctx.manager.connect(function (error) {
    finalCalls++;
    check(
      "child exit during final connected transition fails startup",
      !!error && error.code === "CORE_START_FAILED",
    );
  });
  ctx.resolve(0);
  await later(20);
  check(
    "final transition never reports connected with dead core",
    finalCalls === 1 && ctx.manager.state === "idle",
  );
  check(
    "route rollback and lock release occur once on final transition failure",
    ctx.metrics.rollback === 1 && !ctx.manager.lock.status().held,
  );

  ctx = makeManager({ connectTimeoutMs: 25 });
  var timeoutCalls = 0;
  ctx.manager.connect(function (error) {
    timeoutCalls++;
    check(
      "hung endpoint resolution returns a stable connection timeout",
      !!error && error.code === "CONNECTION_TIMEOUT",
    );
  });
  await later(55);
  check(
    "endpoint timeout settles to idle and permits an immediate retry",
    timeoutCalls === 1 &&
      ctx.manager.state === "idle" &&
      ctx.manager.activeOperation === null &&
  ...
  ctx = makeManager({ connectTimeoutMs: 25, dataPlaneHang: true });
  var probeTimeoutCalls = 0;
  ctx.manager.connect(function (error) {
    probeTimeoutCalls++;
    check(
      "hung data-plane verification returns a connection timeout",
      !!error && error.code === "CONNECTION_TIMEOUT",
    );
  });
  ctx.resolve(0);
  await later(55);
  check(
    "data-plane timeout rolls back routes and releases the lock",
    probeTimeoutCalls === 1 &&
      ctx.manager.state === "idle" &&
  ...
  var source = fs.readFileSync(
    path.join(__dirname, "..", "app", "service", "lib", "vpn", "manager.js"),
    "utf8",
  );
  check(
    "post-route investigative probes are absent from production manager",
    source.indexOf("probeEndpoints") < 0 &&
      source.indexOf("probeCoreBootstrap") < 0 &&
      source.indexOf("coreUsesEndpoint") < 0,
  );
}
...
function readinessParsingTest() {
  ...
  try {
    ...
    check(
      "SOCKS readiness accepts one row with loopback, port and LISTEN state",
      vpnLib.VpnManager.prototype.socksReady.call(manager),
    );
    readinessParsingTest.value = decoy;
    check(
      "SOCKS readiness rejects split-row port/state decoys",
      !vpnLib.VpnManager.prototype.socksReady.call(manager),
    );
  } finally {
...
function coreEnvironmentTest() {
  ...
  vpnLib.VpnManager.prototype.spawnLogged.call(
    manager,
    "sing-box",
    "/fake/sing-box",
    ["run", "-c", "/fake/config"],
  ...
  check(
    "native core launch clears inherited webOS preload",
    !!captured &&
      captured.env.LD_PRELOAD === "" &&
      captured.env.PATH === "/custom/path" &&
      captured.env.EXTRA === "kept",
    JSON.stringify(captured),
  );
  check(
    "native core launch clears inherited webOS platform context",
    !!captured &&
      captured.env.QT_QPA_PLATFORM === "" &&
      captured.env.LS_SERVICE_NAMES === "",
    JSON.stringify(captured),
  );
  check(
    "sing-box launch avoids Node stream attachment on affected webOS runtime",
    !!captured && captured.stdio === "ignore",
    JSON.stringify(captured),
  );
}
...
function diagnosticBoundTest() {
  var logged = [];
  var result = diagnostics.report(
    {
      warn: function (label, meta) {
        logged.push([label, meta]);
      },
    },
    {
      pid: 4242,
      ...
      coreOutputText:
        "secret.example.com 203.0.113.45 vless://uuid:password@host.example/",
    },
  );
  var serialized = JSON.stringify(logged);
  check(
    "production diagnostics retain bounded facts and failure stage",
    result.stage === "core-readiness" &&
      serialized.indexOf("4242") >= 0 &&
      serialized.indexOf("23") >= 0 &&
      serialized.indexOf("SIGABRT") >= 0 &&
      serialized.indexOf("core-readiness") >= 0,
  );
  check(
    "production diagnostics redact endpoint, address and URI data",
    serialized.indexOf("secret.example.com") < 0 &&
      serialized.indexOf("203.0.113.45") < 0 &&
      serialized.indexOf("vless://") < 0,
  );
}

Promise.resolve()
  .then(supervisorIdentityTests)
  .then(managerRaceTests)
  .then(function () {
