...
assert.ok(sourceCount >= 3, 'data-plane probe needs independent fallback sources');
assert.strictEqual(diagnostics.IP_CHECK_URLS[0], 'https://ipinfo.io/ip');
diagnostics.IP_CHECK_URLS.forEach(function (url) {
  assert.strictEqual(url.indexOf('https://'), 0, 'IP probe sources stay HTTPS-only');
});

httpClient.fetchUrl = function (url, options, callback) {
  calls.push(url);
  if (calls.length === 1) return callback(new Error('probe source unavailable'));
...
new diagnostics.Diagnostics({}).externalIp(function (error, address) {
  ...
  assert.strictEqual(calls.length, 3, 'probe stops at the first valid IP response');
