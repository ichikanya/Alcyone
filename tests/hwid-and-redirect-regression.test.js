...
assert.strictEqual(
  ssrf.SENSITIVE_HEADERS.indexOf("x-hwid") >= 0,
  true,
  "X-HWID must be sensitive",
);
assert.strictEqual(
  ssrf.SENSITIVE_HEADERS.indexOf("authorization") >= 0,
  true,
  "Authorization must remain in SENSITIVE_HEADERS",
);
...
console.log(
  "ok 1 - cross-origin stripping covers credentials, X-HWID and every X-Device-* header",
);
...
devInfo.getDeviceInfo(function (err, info) {
  ...
  assert.ok(
    info.ndid,
    "Physical/fallback device info must produce a non-empty ndid",
  );
  ...
  assert.strictEqual(
    hwid.length,
    32,
    "HWID must be a 32-character SHA-256 hash",
  );
  console.log("ok 2 - DeviceInfo returns non-empty 32-char HWID");
});

/* 3. Headers generation: opt-in on HTTPS and forbidden on plaintext. */
var reqOptsEnabled = {
  compatMode: true,
...
assert.strictEqual(
  headersEnabled["X-HWID"],
  "0123456789abcdef0123456789abcdef",
  "HWID-enabled HTTPS request must include X-HWID",
);
...
assert.strictEqual(
  headersDisabled["X-HWID"],
  undefined,
  "ordinary HTTPS requests must not disclose the device HWID",
);
assert.strictEqual(
  headersDisabled["User-Agent"],
  subscriptionsLib.CLIENT_PROFILES[0].ua,
  "ordinary requests must keep the selected client profile",
);
...
assert.strictEqual(
  headersHttp["X-HWID"],
  undefined,
  "Plaintext HTTP request must NOT include X-HWID (HTTPS only policy)",
);
console.log("ok 3 - HWID header generation policy verified");

/* 4. Missing device ID handling */
var emptyDevInfo = new DeviceInfo();
emptyDevInfo._deviceData = {
  ndid: "",
  modelName: "",
  firmwareVersion: "",
  osVersion: "",
};
emptyDevInfo._readPhysicalHardwareId = function () {
  return "";
};
...
assert.strictEqual(
  emptyHwid.length,
  32,
  "Empty raw ID falls back cleanly to hashing default identifier",
);
...
console.log(
  "ok 5 - Provider errors (401, 403, HWID_REQUIRED) are sanitized and distinct from NO_SERVERS_FOUND",
);

/* 6. Web importer has no toggle and accepts the legacy field without exposing it. */
var importerHtml = templates.importerPage("en", {
  profiles: [],
  subscriptions: [],
  csrf: "test",
});
assert.strictEqual(
  importerHtml.indexOf('type="checkbox"'),
  -1,
  "web importer must not render an HWID checkbox",
);
assert.strictEqual(
  importerHtml.indexOf("compatMode"),
  -1,
  "new web importer requests must not expose compatMode",
);
var subscriptionRowHtml = templates.importerPage("en", {
  profiles: [],
  subscriptions: [
    {
      id: "sub-1",
      ...
      count: 3,
      hasProviderHwid: true,
    },
  ],
  csrf: "test",
});
assert.strictEqual(
  subscriptionRowHtml.indexOf("editHwid("),
  -1,
  "subscription rows must not render an inline HWID editor",
);
assert.strictEqual(
  subscriptionRowHtml.indexOf("row.hwidSet"),
  -1,
  "subscription rows must not render a change-HWID button",
);
assert.ok(
  subscriptionRowHtml.indexOf("updateSub('sub-1')") >= 0 &&
    subscriptionRowHtml.indexOf("delSub('sub-1')") >= 0,
  "subscription rows keep only the compact update and delete actions",
);
...
server.dispatch(
  "/api/import",
  { value: "https://example.com/sub", name: "TestSub", compatMode: false },
  function (err, res) {
    assert.ifError(err);
    ...
    assert.strictEqual(
      mockCompatValue,
      true,
      "ImporterServer must override a legacy false value",
    );
    console.log(
      "ok 6 - Web Import hides the legacy compatibility field",
    );
  },
);
...
assert.strictEqual(
  loggedLine.indexOf("0123456789abcdef0123456789abcdef"),
  -1,
  "HWID value must be scrubbed from log",
);
assert.strictEqual(
  loggedLine.indexOf("secret_token_12345"),
  -1,
  "Secret token must be scrubbed from log",
);
