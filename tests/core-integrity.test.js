...
var integrity = require(path.join(ROOT, 'app', 'service', 'lib', 'core-integrity.js'));
...
assert.deepStrictEqual(integrity.PINNED_SHA256, recorded,
  'runtime checksum pins must exactly match the build provenance metadata');
console.log('ok 1 - runtime SHA-256 pins match provenance for XRay, tun2socks and sing-box');

Object.keys(sources).forEach(function (name) {
  assert.ok(fs.existsSync(sources[name]), 'missing generated test core: ' + name);
  assert.strictEqual(integrity.sha256File(sources[name]), integrity.PINNED_SHA256[name]);
});
...
assert.strictEqual(integrity.sha256File(staged), integrity.PINNED_SHA256.tun2socks);
if (process.platform !== 'win32') assert.ok((fs.statSync(staged).mode & 73) !== 0);
console.log('ok 3 - a missing staged core is atomically restored from a verified package core');

var damaged = fs.readFileSync(staged);
damaged[damaged.length - 1] = damaged[damaged.length - 1] ^ 1;
fs.writeFileSync(staged, damaged);
assert.strictEqual(fs.statSync(staged).size, fs.statSync(packaged).size,
  'fixture must reproduce the prior same-size staging bypass');
assert.strictEqual(integrity.prepare(packaged, staged, 'tun2socks'), staged);
assert.strictEqual(integrity.sha256File(staged), integrity.PINNED_SHA256.tun2socks);
assert.ok(!fs.existsSync(staged + '.integrity-' + String(process.pid || 0) + '.tmp'));
console.log('ok 4 - a same-size corrupt stage is replaced atomically and leaves no temporary file');
...
console.log('ok 5 - a packaged checksum mismatch fails closed without touching the stage');
...
console.log('ok 6 - final pre-spawn verification rejects post-staging tampering');

var verifyCalls = [];
var managerLib = require(path.join(ROOT, 'app', 'service', 'lib', 'vpn', 'manager.js'));
var manager = new managerLib.VpnManager({
  edition: { core: 'sing-box' },
  paths: { appDir: '/app', dataDir: '/data', tunnelLog: path.join(tmp, 'tunnel.log') },
  logger: { info: function () {}, warn: function () {}, error: function () {} },
  coreIntegrity: {
    prepare: function (source, target) { return target; },
    verifyForLaunch: function (file, name) {
      verifyCalls.push(name + ':' + file);
      throw require(path.join(ROOT, 'app', 'service', 'lib', 'errors.js'))
        .err('CORE_INTEGRITY_FAILED', name);
    }
  }
});
