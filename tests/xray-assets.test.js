...
var serviceSource = fs.readFileSync(path.join(__dirname, '..', 'app', 'service', 'service.js'), 'utf8');

assert.ok(
  /path\.resolve\(__dirname,\s*'\.\.',\s*'\.\.',\s*'applications',\s*edition\.appId\)/.test(serviceSource),
...
var root = path.join(os.tmpdir(), 'alcyone-xray-assets-' + process.pid + '-' + Date.now());
...
try {
  ...
  fs.copyFileSync(path.join(staged, 'geosite.dat'), path.join(appBin, 'geosite.dat'));
  fs.copyFileSync(path.join(staged, 'geoip.dat'), path.join(appBin, 'geoip.dat'));
  ...
  assert.strictEqual(fs.readFileSync(path.join(dataBin, 'xray'), 'utf8'), 'persisted-xra');
  assert.strictEqual(fs.readFileSync(path.join(dataBin, 'tun2socks'), 'utf8'), 'packaged-tun2socks');

  assert.deepStrictEqual(migrator.installBundledXrayAssets(), ['geoip.dat', 'geosite.dat']);
  ...
  assert.strictEqual(hash(path.join(dataBin, 'geosite.dat')), xrayAssets.ASSETS['geosite.dat'].sha256);
  ...
  manager.resolveCores = function () {
    return { xray: path.join(dataBin, 'xray'), tun2socks: path.join(dataBin, 'tun2socks') };
  };
  ...
  manager.resolveEndpoints = function (profile, callback) {
    callback(null, {
      entries: [{ host: 'redacted.test', addresses: ['93.184.216.34'], targets: [{ port: 443, network: 'tcp' }] }],
  ...
  console.log('ok - pinned Xray assets install, verify, repair, and fail precisely');
} finally {
  removeTree(root);
}
