...
function makeElement(id) {
  return {
    ...
    style: {}, childNodes: [], scrollTop: 0, scrollHeight: 0, clientHeight: 0,
...
['stateText', 'hint', 'power', 'current', 'homeStage', 'serverList', 'count', 'search',
 ...
 'aboutVersion', 'webUrl', 'webHint'].forEach(function (id) { elements[id] = makeElement(id); });
...
function fireDeadlines() {
  var due = timers.filter(function (t) { return t.ms >= 10000; });
  timers = timers.filter(function (t) { return t.ms < 10000; });
...
var windowStub = {
  ALCYONE_EDITION: {
    ...
    coreLabel: 'XRay', editionName: 'XRay Edition', title: 'Alcyone XRay', version: '4.0.1'
  },
...
record('frontend uses PalmServiceBridge when window.webOS is absent', bridges.length > 0,
  bridges.length + ' bridge(s)');
record('bridge is addressed with the scoped service URI',
  bridges.length > 0 && String(bridges[0].calledWith.uri).indexOf('luna://com.alcyone.vpn.service/') === 0,
...
record('an in-flight bridge keeps a live onservicecallback reference',
...
record('log refresh disables its button and shows a loading label',
  logsButton.disabled === true && loadingLabel.length > 0, loadingLabel);
record('log refresh issued a getLogs call over the bridge',
  bridges.length === bridgeCountBeforeLogs + 1 &&
  bridges[bridges.length - 1].calledWith.uri.indexOf('/getLogs') > 0);

var fired = fireDeadlines();
record('a call with no reply eventually hits its deadline', fired > 0, fired + ' deadline(s)');
record('log refresh button recovers instead of staying on "Loading..."',
  logsButton.disabled === false && logsButton.textContent !== loadingLabel,
  logsButton.textContent);
record('the user is told the service is not answering',
...
logsButton.onclick();
record('a second refresh is not blocked by a stale busy flag',
...
record('"Allow for 5 minutes" issues startPairing',
  bridges.length === pairBridges + 1 &&
  bridges[bridges.length - 1].calledWith.uri.indexOf('/startPairing') > 0,
  bridges.length > pairBridges ? bridges[bridges.length - 1].calledWith.uri : 'no call');
fireDeadlines();
record('a silent startPairing failure surfaces to the user rather than hanging',
  elements.log.textContent.indexOf('elevate-service') >= 0);
...
try {
  if (late.onservicecallback) {
    late.onservicecallback(JSON.stringify({ returnValue: true, ok: true, code: 'LATE' }));
  }
} catch (e) { threw = true; }
