...
var source = fs
  .readFileSync(path.join(ROOT, "app", "app.js"), "utf8")
  .replace(
    "var ELEVATION_POLL_INTERVAL_MS = 1000;",
    "var ELEVATION_POLL_INTERVAL_MS = 5;",
  )
...
function boot(scenario, done) {
  ...
  function respond(uri, method, params, onSuccess, onFailure) {
    ...
    if (uri === "luna://org.webosbrew.hbchannel.service") {
      if (method === "checkRoot") {
        if (scenario.checkRootFails)
          return onFailure({ errorCode: "SERVICE_UNAVAILABLE" });
    ...
    if (method === "getState") {
      var elevated =
        !!scenario.initialRoot ||
        (scenario.rootAfterPolls !== undefined &&
          pollCount >= scenario.rootAfterPolls &&
          scenario.restarted);
      ...
      return onSuccess({
        ...
        privilege: elevated
          ...
          : {
              uid: 5033,
              root: !!scenario.rootTrueWithoutUid,
              pid: 5586,
              appPayloadReadable: true,
    ...
    if (method === "restartService") {
      if (scenario.restartFails)
        return onFailure({ errorCode: "RESTART_FAILED" });
  ...
  var context = {
    ...
    setTimeout: function (fn, delay) { return setTimeout(fn, delay === 1000 ? 5 : delay); },
...
function countMethod(run, name) {
  return methods(run).filter(function (method) {
    return method === name;
  }).length;
}
...
function runHealthyScenario() {
  boot({ initialRoot: true }, function (healthy) {
    record(
      "an Alcyone service already running as uid 0 does not query Homebrew",
      countMethod(healthy, "checkRoot") === 0,
    );
    record(
      "an Alcyone service already running as uid 0 is never elevated or restarted",
      countMethod(healthy, "elevateService") === 0 &&
        countMethod(healthy, "restartService") === 0,
    );
...
function runAutomaticEdition(edition, next) {
  boot(
    ...
    function (automatic) {
      record(
        edition + " checks Homebrew root exactly once before elevation",
        countMethod(automatic, "checkRoot") === 1,
      );
      record(
        edition + " begins elevation without a button press",
        countMethod(automatic, "elevateService") === 1,
      );
      record(
        edition + " shows a neutral temporary status",
        automatic.elements.elevationTitle.textContent ===
          "Preparing permissions" &&
          automatic.elements.elevationText.textContent.indexOf(
            "Preparing permissions",
          ) >= 0,
        automatic.elements.elevationTitle.textContent,
      );
      record(
        edition + " hides the manual fallback during the automatic attempt",
        automatic.elements.grantPermissions.style.display === "none",
      );
      record(
        edition + " disables VPN activation during the automatic attempt",
      ...
      setTimeout(function () {
        ...
        var elevationSequence = sequence
          .slice(elevateIndex)
        ...
        record(
          edition + " runs elevateService before restartService",
          elevateIndex >= 0 &&
        ...
        record(
          edition + " elevates only its fixed edition service id",
          targetCall &&
            targetCall.parameters.id === automatic.edition.serviceId &&
            Object.keys(targetCall.parameters).length === 1,
          targetCall && JSON.stringify(targetCall.parameters),
        );
        record(
          edition + " polls getState and finishes only after uid 0",
...
function runUnsupportedScenario() {
  boot({ homebrewRoot: false }, function (unsupported) {
    record(
      "Homebrew without root keeps the unsupported-environment message",
      unsupported.elements.elevationTitle.textContent ===
        "A rooted TV is required",
      unsupported.elements.elevationTitle.textContent,
    );
    record(
      "Homebrew without root never attempts elevation",
      countMethod(unsupported, "elevateService") === 0 &&
        countMethod(unsupported, "restartService") === 0,
    );
    record(
      "Homebrew without root offers no manual elevation fallback",
...
function runUnknownScenario() {
  boot({ checkRootFails: true }, function (unknown) {
    record(
      "an unavailable Homebrew root verdict remains neutral and does not elevate",
    ...
    record(
      "an unavailable Homebrew root verdict offers no fallback action",
...
function runFailureAndRetryScenario() {
  boot({ homebrewRoot: true, elevateFails: true }, function (failed) {
    record(
      "a failed automatic attempt exposes the manual fallback",
    ...
    record(
      "a failed automatic attempt shows a concise fallback message",
      failed.elements.elevationText.textContent.indexOf(
        "Could not grant permissions",
      ) >= 0 && failed.elements.elevationText.textContent.indexOf("DENIED") < 0,
      failed.elements.elevationText.textContent,
    );
    record(
      "a failed elevateService never restarts Alcyone",
    ...
    setTimeout(function () {
      record(
        "runtime refresh cannot loop the automatic attempt",
      ...
      setTimeout(function () {
        record(
          "the manual fallback can retry after automatic failure",
...
function runBoundedTimeoutScenario() {
  boot({ homebrewRoot: true, observeAfterMs: 350 }, function (timedOut) {
    record(
      "automatic polling stops at the configured attempt bound",
      countMethod(timedOut, "getState") === 16,
      String(countMethod(timedOut, "getState")),
    );
    record(
      "a bounded polling timeout exposes the manual fallback",
      timedOut.elements.elevationText.textContent.indexOf(
        "did not come back as root in time",
      ) >= 0 &&
