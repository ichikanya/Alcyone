...
assert.strictEqual(
  profiles.length,
  2,
  "a complete Xray config is one profile, not one profile per outbound",
);
...
assert.strictEqual(
  parser.parseProxyLink(profiles[0].link).params.serviceName,
  "alpha-service",
);
...
var encodedTrojan =
  "trojan://pa%23ss%3Fx@trojan.example.test:443?type=ws&path=%2Fws%3Fed%3D2048%26x%3D1#Encoded";
profiles = parser.extractSubscriptionProfiles(encodedTrojan);
assert.strictEqual(
  profiles.length,
  1,
  "encoded reserved characters must not make a valid URI disappear",
);
assert.strictEqual(parser.parseProxyLink(profiles[0].link).password, "pa#ss?x");
assert.strictEqual(
  parser.parseProxyLink(profiles[0].link).params.path,
  "/ws?ed=2048&x=1",
);
...
var grpcVlessA =
  "vless://" +
  grpcUuid +
  "@grpc.example.test:443?security=tls&type=grpc&serviceName=alpha#Alpha";
var grpcVlessB =
  "vless://" +
  grpcUuid +
  "@grpc.example.test:443?security=tls&type=grpc&serviceName=beta#Beta";
profiles = parser.extractSubscriptionProfiles(grpcVlessA + "\n" + grpcVlessB);
assert.strictEqual(
  profiles.length,
  2,
  "distinct VLESS gRPC services must not be deduplicated",
);
...
var sameNameTcp =
  "vless://" +
  transportUuid +
  "@transport.example.test:443?security=tls&type=tcp#Same";
var sameNameXhttp =
  "vless://" +
  transportUuid +
  "@transport.example.test:443?security=tls&type=xhttp&path=%2Fxhttp&mode=auto#Same";
profiles = parser.extractSubscriptionProfiles(
  sameNameTcp + "\n" + sameNameXhttp,
);
assert.strictEqual(
  profiles.length,
  2,
  "same-name VLESS nodes with distinct TCP and XHTTP transports must both be imported",
);

var grpcTrojanA =
  "trojan://secret@grpc.example.test:443?type=grpc&serviceName=alpha#Alpha";
var grpcTrojanB =
  "trojan://secret@grpc.example.test:443?type=grpc&serviceName=beta#Beta";
profiles = parser.extractSubscriptionProfiles(grpcTrojanA + "\n" + grpcTrojanB);
assert.strictEqual(
  profiles.length,
  2,
  "distinct Trojan gRPC services must not be deduplicated",
);

var singleSocks = "socks5://user:pass@socks.example.test:1080#SOCKS";
profiles = parser.extractSubscriptionProfiles(
  Buffer.from(singleSocks, "utf8").toString("base64"),
);
assert.strictEqual(
  profiles.length,
  1,
  "a single base64-encoded SOCKS5 node must decode",
);
...
assert.strictEqual(
  profiles.length,
  3,
  "JSON protocol aliases and SIP008 records must be imported",
);
...
assert.strictEqual(
  profiles.length,
  1,
  "nested YAML lists must not terminate a Clash proxy",
);
assert.strictEqual(
  parser.parseProxyLink(profiles[0].link).params.serviceName,
  "nested-service",
...
assert.strictEqual(
  parser.parseProxyLink(profiles[0].link).params.alpn,
  "h2,http/1.1",
);
...
assert.strictEqual(profiles.length, 1);
assert.strictEqual(
  parser.parseProxyLink(profiles[0].link).params.host,
  "inline-origin.example.test",
);
...
var wsHosts = profiles
  .map(function (p) {
    return parser.parseProxyLink(p.link).params.host;
  })
  .sort();
assert.deepStrictEqual(wsHosts, [
  "trojan-origin.example.test",
  "vless-origin.example.test",
  "vmess-origin.example.test",
]);
