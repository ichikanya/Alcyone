...
var CLIENT = path.join(__dirname, '..', 'app', 'service', 'lib', 'net', 'http-client.js');
var ssrf = require(path.join(__dirname, '..', 'app', 'service', 'lib', 'net', 'ssrf.js'));
...
dns.lookup = function (hostname, options, callback) {
  if (hostname === TEST_HOST || hostname === ALT_HOST) {
    ...
    if (options && options.all) return process.nextTick(function () { callback(null, [{ address: FAKE_PUBLIC, family: 4 }]); });
    return process.nextTick(function () { callback(null, FAKE_PUBLIC, 4); });
  }
...
dns.resolve4 = function (hostname, callback) {
  if (hostname === TEST_HOST || hostname === ALT_HOST) {
    ...
    return process.nextTick(function () { callback(null, [FAKE_PUBLIC]); });
  }
...
dns.resolve6 = function (hostname, callback) {
  if (hostname === TEST_HOST || hostname === ALT_HOST) {
    ...
    noData.code = 'ENODATA';
    return process.nextTick(function () { callback(noData); });
  }
...
var server = http.createServer(function (req, res) {
  ...
  if (req.url === '/plain') {
    ...
    return res.end('vless://11111111-2222-3333-4444-555555555555@a.ex.com:443#A');
  }
  if (req.url === '/gzip') {
    var body = zlib.gzipSync(new Buffer('vless://11111111-2222-3333-4444-555555555555@b.ex.com:443#B'));
  ...
  if (req.url === '/redirect-chain') {
    res.writeHead(302, { Location: 'http://' + ALT_HOST + '/redirect-chain-mid' });
    return res.end();
  }
...
server.listen(0, '127.0.0.1', function () {
  ...
  step(function (next) {
    client.fetchUrl(base + '/plain', {}, function (e, body) {
      record('plain body downloads', !e && /vless:\/\//.test(String(body)), e ? e.code : '');
      next();
    });
  }).then(function () {
    return step(function (next) {
      client.fetchUrl(base + '/gzip', {}, function (e, body) {
        record('gzip response decompresses', !e && /b\.ex\.com/.test(String(body)), e ? e.code : '');
        next();
      });
    });
  }).then(function () {
    return step(function (next) {
      received.length = 0;
      client.fetchUrl(base + '/redirect-same', {
        headers: { 'Authorization': 'Bearer SECRET', 'X-Hwid': 'HW1', 'User-Agent': 'Happ/3.1.0' }
      }, function (e, body) {
        var followed = received.length === 2;
        var keptAuth = followed && !!received[1].headers.authorization;
        record('same-origin redirect is followed', !e && followed && /a\.ex\.com/.test(String(body)), e ? e.code : '');
    ...
    return step(function (next) {
      received.length = 0;
      client.fetchUrl(base + '/redirect-cross', {
        headers: {
        ...
        var stripped = hopped && !second.authorization && !second.cookie &&
          !second['proxy-authorization'] && !second['x-hwid'] &&
          !second['x-device-os'] && !second['x-device-custom'];
        var keptUa = hopped && !!second['user-agent'];
        record('cross-origin redirect strips credential headers', !e && stripped, e ? e.code : (hopped ? '' : 'not followed'));
    ...
    return step(function (next) {
      ...
      client.fetchUrl(base + '/redirect-chain', {
        headers: { Authorization: 'Bearer SECRET', 'X-HWID': 'HW1', 'X-Device-model': 'TV' }
      }, function (e, body) {
        var third = received.length === 3 ? received[2].headers : {};
        var revalidated = resolvedHosts.indexOf(TEST_HOST) >= 0 && resolvedHosts.indexOf(ALT_HOST) >= 0;
        record('redirect chain revalidates DNS and follows every hop',
          !e && received.length === 3 && revalidated && /a\.ex\.com/.test(String(body)), e ? e.code : '');
        record('headers stripped on an origin change stay stripped for the rest of the chain',
          !third.authorization && !third['x-hwid'] && !third['x-device-model']);
        next();
      });
    });
  }).then(function () {
    return step(function (next) {
      received.length = 0;
      client.fetchUrl(base + '/redirect-port', {
        headers: { Authorization: 'Bearer SECRET', Cookie: 'sid=abc' }
      }, function (e) {
        var second = received.length === 2 ? received[1].headers : {};
        record('effective port participates in cross-origin header stripping',
          !e && !second.authorization && !second.cookie, e ? e.code : '');
        next();
      });
    });
  }).then(function () {
    return step(function (next) {
      client.fetchUrl(base + '/redirect-private', {}, function (e) {
        record('redirect to private address is revalidated and blocked',
          !!e && e.code === 'BLOCKED_ADDRESS', e ? e.code + '/' + e.detail : 'no error');
        record('redirect SSRF failure includes safe hop diagnostics',
          !!e && e.meta && e.meta.stage === 'redirect' &&
          e.meta.redirectHop === 1 && e.meta.protocol === 'http' &&
    ...
    return step(function (next) {
      ...
      client.fetchUrl(secretUrl, {}, function (e) {
        ...
        record('connection reset identifies its safe request stage',
          !!e && e.code === 'NETWORK_ERROR' && e.meta &&
          (e.meta.stage === 'connect' || e.meta.stage === 'response-read') &&
          e.meta.redirectHop === 0 && e.meta.protocol === 'http' &&
        ...
        record('network diagnostics contain no URL, host, path, query or token',
    ...
    return step(function (next) {
      client.fetchUrl(base + '/redirect-loop', {}, function (e) {
        record('redirect loop is bounded', !!e && e.code === 'TOO_MANY_REDIRECTS', e ? e.code : 'no error');
        next();
      });
    });
  }).then(function () {
    return step(function (next) {
      client.fetchUrl(base + '/huge', {}, function (e) {
        record('oversized body is rejected', !!e && e.code === 'RESPONSE_TOO_LARGE', e ? e.code : 'no error');
        next();
      });
    });
  }).then(function () {
    return step(function (next) {
      client.fetchUrl(base + '/zipbomb', {}, function (e) {
        record('decompression bomb is rejected',
          !!e && (e.code === 'DECOMPRESSED_TOO_LARGE' || e.code === 'RESPONSE_TOO_LARGE' || e.code === 'NETWORK_ERROR'),
          e ? e.code : 'no error');
        next();
      });
    });
  }).then(function () {
    var httpsTarget = ssrf.assertUrlAllowed('https://' + TEST_HOST + '/secure');
    var downgrade = null;
    try {
    ...
    record('HTTPS to HTTP redirect downgrade is rejected',
      !!downgrade && downgrade.code === 'HTTPS_DOWNGRADE_REJECTED',
      downgrade ? downgrade.code : 'no error');
    record('downgrade diagnostics identify the redirect without destination data',
      downgrade && downgrade.meta && downgrade.meta.stage === 'redirect' &&
      downgrade.meta.protocol === 'http' && downgrade.meta.originChanged === true &&
      JSON.stringify(downgrade.meta).indexOf(TEST_HOST) < 0);
    record('same origin uses scheme, hostname and effective port',
      ssrf.sameOrigin(
...
function runTlsCheck() {
  return new Promise(function (resolve) {
    ...
    if (!selfSigned) {
      record('self-signed TLS is rejected without insecure retry (skipped: no test cert)', true, 'skipped');
      return resolve();
    }
    ...
    tlsServer.listen(0, '127.0.0.1', function () {
      ...
      https.request = function (options, cb) {
        ...
        if (options && options.host === FAKE_PUBLIC) { options.host = '127.0.0.1'; options.port = tlsPort; }
        return realHttpsRequest.call(https, options, cb);
      };
      client.fetchUrl('https://' + TEST_HOST + '/plain', {}, function (e, body) {
        record('self-signed TLS is rejected without insecure retry',
          !!e && e.code === 'TLS_CERTIFICATE_INVALID' && !body &&
          e.meta && e.meta.transportErrorName === 'Error' &&
          /^(tcp-connect|tls-handshake|certificate-verification)$/.test(e.meta.tlsPhase),
          e ? e.code + '/' + (e.meta && e.meta.tlsPhase) : 'no error');
        record('supported Node runtimes advertise the safe P-256/P-384/P-521 compatibility list',
          tlsOptionsSeen &&
