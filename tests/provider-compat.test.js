...
deviceInfo.getHwid(function (err, hwid) {
  ...
  console.log('ok   - headersFor injects X-HWID and Happ metadata for HTTPS with compatMode=true');
  ...
  api.addSubscription({ url: FIXTURE_URL, name: 'Fixture Provider Test', compatMode: true }, function (addErr, addRes) {
    ...
    api.addSubscription({ url: FIXTURE_URL, name: 'Ordinary Fixture Test', compatMode: false }, function (ordinaryErr, ordinaryRes) {
      ...
      subscriptions.expandNested(NESTED_URL, {
        headers: subscriptions.headersFor(0, { compatMode: false, isHttps: true }),
        deadline: Date.now() + 1000
      }, function (nestedErr, nestedBody) {
        assert.ifError(nestedErr);
        assert.ok(nestedBody.indexOf('nested.example.com') >= 0);
        console.log('ok   - nested subscription fetch preserves the secure ordinary request behavior');
        ...
        api.addSubscription({
          ...
          if (liveErr) console.log('# optional live compatibility check skipped: ' + liveErr.code);
          else console.log('# optional live compatibility check: ' + liveRes.count + ' profiles');
          done();
        });
      });
    });
