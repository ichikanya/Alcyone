...
var endpointBootstrap = require(path.join(ROOT, 'app', 'service', 'lib', 'net', 'endpoint-bootstrap.js'));
...
var xrayConfig = require(path.join(ROOT, 'app', 'service', 'lib', 'config', 'xray.js'));
var singboxConfig = require(path.join(ROOT, 'app', 'service', 'lib', 'config', 'singbox.js'));
...
function dnsCheck(next) {
  ...
  dns.resolve4 = function (host, callback) {
    process.nextTick(function () { callback(null, ['93.184.216.34']); });
  };
  dns.resolve6 = function (host, callback) {
    process.nextTick(function () { callback(new Error('no AAAA')); });
  };
  dnsResolver.resolveAll('webos4.test', function (error, addresses) {
    dns.resolve4 = real4;
    dns.resolve6 = real6;
    check('DNS result shape works on Node 0.12.2',
...
function decompressionCheck(next) {
  ...
  zlib.gzip(large, function (gzipError, compressed) {
    ...
    httpClient.decodeBody(compressed, 'gzip', function (error) {
      check('decompression limit works on Node 0.12.2',
...
function nestedFailureCheck(next) {
  ...
  subscriptions.expandNested(
    ...
    function (error) {
      ...
      check('nested failure completes on Node 0.12.2',
...
function httpsRedirectCheck(next) {
    ...
    request.end = function () {
      process.nextTick(function () {
        ...
        if (options.path === '/start') {
          ...
          response.headers = { location: 'https://redirect-target.example/finish' };
          callback(response);
          return;
        }
  ...
  httpClient.fetchUrl('https://redirect-source.example/start', {
    headers: { Authorization: 'Bearer secret', 'User-Agent': 'Happ/3.1.0/android' }
  }, function (error, body) {
    dns.resolve4 = real4;
    dns.resolve6 = real6;
    https.request = realRequest;
    check('HTTPS redirect chain works on Node 0.12.2',
      !error && requests.length === 2 && body === 'vless://runtime.example');
    check('approved IPv4 is pinned while Host and SNI keep the original name on Node 0.12.2',
      requests.length === 2 &&
    ...
    check('cross-origin credentials are stripped on Node 0.12.2',
      requests.length === 2 && !requests[1].headers.Authorization &&
      !requests[1].headers.authorization);
    next();
  });
}

check('bundled Mozilla CA roots load on Node 0.12.2',
  httpClient.loadBundledCa() && httpClient.loadBundledCa().length === 119);
var runningNode012 = /^0\.12\./.test(String(process.versions.node || ''));
check('runtime selects the compatible ECDH curve policy',
  runningNode012
...
check('transport diagnostics are useful and secret-safe on Node 0.12.2',
...
check('redirect resolution rejects HTTPS downgrade on Node 0.12.2',
