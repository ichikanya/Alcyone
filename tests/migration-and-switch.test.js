...
function tempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "alcyone-mig-"));
}
...
record(
  "migration preserves every profile",
  after.profiles.length === 3,
  String(after.profiles.length),
);
record(
  "migration preserves profile ids",
  ["pold1", "pold2", "pold3"].every(function (id) {
    return after.profiles.some(function (p) {
      return p.id === id;
    });
  }),
);
...
record(
  "migration preserves proxy links verbatim",
  after.profiles[0].link === LEGACY.profiles[0].link,
);
record(
  "migration preserves the full Xray config",
  !!after.profiles[2].fullConfig &&
    after.profiles[2].fullConfig.outbounds[0].streamSettings.network ===
      "xhttp",
);
record(
  "migration preserves subscriptions",
  after.subscriptions.length === 1 &&
    after.subscriptions[0].url === LEGACY.subscriptions[0].url,
);
...
record(
  ...
  JSON.stringify(store.read()) === revisionBefore,
);
record(
  "migration records its version",
  JSON.parse(fs.readFileSync(paths.stateFile, "utf8")).migrationVersion ===
    migrateLib.MIGRATION_VERSION,
);
...
fs.writeFileSync(
  ...
  JSON.stringify([
    {
      id: "a1",
      link: "vless://11111111-2222-3333-4444-555555555555@d.example.com:443#One",
      name: "One",
    },
  ]),
);
new migrateLib.Migrator({
  paths: paths2,
  edition: { id: "xray", core: "xray" },
  logger: quiet,
}).run();
...
record(
  "a legacy array-format store is upgraded",
  upgraded.profiles.length === 1 && !!upgraded.activeId,
);
...
record(
  "an interrupted write is recovered from the temp file",
  recovered.profiles.length === 3,
);

/* --- server switch ordering --- */
var switchDir = tempDir();
var switchStore = new storeLib.ProfileStore({
  file: path.join(switchDir, "profiles.json"),
});
var first = switchStore.upsertManualProfile(
  "vless://11111111-2222-3333-4444-555555555555@e.example.com:443#E",
  "E",
);
var second = switchStore.upsertManualProfile(
  "vless://11111111-2222-3333-4444-555555555555@f.example.com:443#F",
  "F",
);
...
api.selectProfile(
  { profileId: first.profile.id, reconnect: true },
  function (error) {
    record(
      "a connected server switch persists the selection first",
      !error && switchStore.read().activeId === first.profile.id,
      error ? error.code : "",
    );
    record(
      "a connected server switch issues exactly disconnect then connect",
      sequence.join(",") === "disconnect,connect",
      sequence.join(","),
    );
    ...
    api.selectProfile(
      { profileId: second.profile.id, reconnect: true },
      function (error2) {
        record(
          "selecting a server while disconnected does not start the VPN",
          !error2 && sequence.length === 0,
          sequence.join(","),
        );
        ...
        api.selectProfile(
          { profileId: second.profile.id, reconnect: false },
          function (error3) {
            record(
              "selecting without reconnect leaves the tunnel untouched",
              !error3 && sequence.length === 0,
              sequence.join(","),
            );
