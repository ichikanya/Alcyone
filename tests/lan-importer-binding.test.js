...
var serviceSource = fs.readFileSync(path.join(ROOT, 'app', 'service', 'service.js'), 'utf8');
assert.ok(/importer\.listen\(false,/.test(serviceSource),
...
server.listen(false, function (loopbackError) {
  ...
  server.listen(true, function (closedLanError) {
    ...
    console.log('ok 3 - LAN bind cannot be enabled without an active temporary window');
    ...
    server.listen(true, function (lanError) {
      ...
      console.log('ok 4 - explicit pairing activation temporarily enables LAN scope');
      ...
      server.enforceExposure(function (expiryError) {
        ...
        assert.strictEqual(server.boundHost, '127.0.0.1');
        console.log('ok 5 - expired pairing/session access automatically returns to loopback');
