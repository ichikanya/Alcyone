...
var IDS = ['stateText', 'hint', 'power', 'current', 'homeStage', 'webUrl', 'webHint', 'webPairInfo', 'webCode', 'webExpiry',
  'pairState', 'pairBox', 'langState', 'autostartState', 'log', 'list', 'srvCount',
  'probeBtn', 'subBtn', 'search', 'sortState', 'coreState', 'verState'];
IDS.forEach(el);
...
var document = {
  ...
  querySelectorAll: function (sel) {
    ...
    if (sel === '[data-i18n]') {
      return Object.keys(elements).map(function (k) { return elements[k]; })
        .filter(function (node) { return node.getAttribute('data-i18n') !== null; });
    }
    return [];
  },
...
var context = {
  ...
  btoa: function (v) { return Buffer.from(v, 'binary').toString('base64'); },
  atob: function (v) { return Buffer.from(v, 'base64').toString('binary'); },
  escape: escape,
  unescape: unescape
};
...
var source = fs.readFileSync(appPath, 'utf8').replace(/\}\)\(\);\s*$/,
  'window.__t = {\n' +
  '  setLang: function (v) { langSetting = v; },\n' +
...
assert.doesNotThrow(function () { t.setLang('ru'); t.updateLangUi(); },
  'updateLangUi must not throw: a throw leaves the language row stale');
checks++;

var ruLabel = el('langState').textContent;
ok(ruLabel && ruLabel.length > 0, 'language row must show a value after switching to Russian');
eq(ruLabel, t.langLabel(), 'language row must match the active language label');

t.setLang('en');
t.updateLangUi();
var enLabel = el('langState').textContent;
ok(enLabel && enLabel.length > 0, 'language row must show a value after switching to English');
ok(enLabel !== ruLabel, 'language row label must change when the language changes');

/* The rest of the screen must be translated in the same pass. */
eq(el('stateText').textContent, t.tr('common.checking'),
...
eq(el('webPairInfo').style.display, 'none',
...
t.setPairing({ code: 'ABCD-1234', port: 8080, addresses: ['10.0.0.7'], secondsRemaining: 120 });
t.setLang('ru');
assert.doesNotThrow(function () { t.updateLangUi(); },
  'updateLangUi must not throw while a pairing window is open');
checks++;
eq(el('webUrl').textContent, 'http://10.0.0.7:8080',
  'a language switch must not wipe the live pairing address');
ok(el('langState').textContent === t.langLabel(),
...
indexHtml.replace(/aria-label="([^"]*)"/g, function (whole, value, offset) {
  ...
  var tag = indexHtml.slice(indexHtml.lastIndexOf('<', offset), offset + whole.length);
  ok(tag.indexOf('data-i18n-aria=') >= 0,
...
indexHtml.replace(/placeholder="([^"]*)"/g, function (whole, value, offset) {
  if (!CYRILLIC.test(value)) return whole;
  var tag = indexHtml.slice(indexHtml.lastIndexOf('<', offset), offset + whole.length);
  ok(tag.indexOf('data-i18n-ph=') >= 0,
    'Russian placeholder must carry data-i18n-ph: ' + value);
  return whole;
});

/* applyI18n must localize aria labels, not just text and placeholders. */
ok(/\[data-i18n-aria\]/.test(
  require('fs').readFileSync(require('path').join(__dirname, '..', 'app', 'app.js'), 'utf8')),
