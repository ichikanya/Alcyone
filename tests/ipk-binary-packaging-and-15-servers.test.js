  ...
  "https://fixture.example.test/active": {
    ...
    headers: {
      ...
      "subscription-userinfo":
        "upload=0; download=0; total=107374182400; expire=4070908800",
    },
  },
  "https://fixture.example.test/expired": {
    body: fs.readFileSync(
      path.join(__dirname, "fixtures", "subscription-expired.txt"),
      "utf8",
    ),
    headers: {
      "content-type": "text/plain; charset=utf-8",
...
httpClient.fetchUrl = function (url, options, callback) {
  ...
  process.nextTick(function () {
    callback(null, fixture.body, fixture.headers);
  });
};
...
subscriptions.download(
  ...
  function (err, result) {
    ...
    assert.strictEqual(
      ...
      15,
      "The active subscription fixture MUST yield exactly 15 unique servers",
    );

    var names = result.imported.map(function (p) {
      return p.name;
    });
    assert.ok(
      ...
      "All server names must be non-empty and valid",
    );
    assert.strictEqual(
      names.length,
      Object.keys(
        names.reduce(function (acc, n) {
          acc[n] = 1;
          return acc;
        }, {}),
      ).length,
    ...
    assert.strictEqual(
      result.clients,
      "Happ",
      "Primary client candidate Happ must be used without redundant merging",
    );
    assert.ok(
      !/expire/i.test(parsers.profileTitleFromHeaders(result.headers, "")),
      "An active subscription must not carry an expiry message",
    );

    console.log(
      "ok 1a - active subscription fixture imported exactly 15 unique servers with clean names",
    );
    subscriptionChecks++;
  },
);
...
subscriptions.download(
  ...
  function (err, result) {
    ...
    assert.strictEqual(
      result.imported.length,
      1,
      "The expired subscription fixture yields exactly one notice profile",
    );
    assert.strictEqual(
      result.imported[0].name,
      "Подписка истекла",
      "The expired-subscription notice must be preserved verbatim as the profile name",
    );

    assert.strictEqual(
      parsers.profileTitleFromHeaders(result.headers, ""),
    ...
    assert.ok(
      userinfo,
      "subscription-userinfo must be merged from the response headers",
    );
    var expire = Number((/expire=(\d+)/.exec(userinfo) || [])[1]);
    assert.ok(
      expire > 0,
      "subscription-userinfo must carry a parseable expire timestamp",
    );
    assert.ok(
      expire * 1000 < Date.now(),
      "The expired fixture must parse as expired, not as active",
    );

    console.log(
      "ok 1b - expired subscription fixture parsed into its notice profile and expiry message",
    );
    subscriptionChecks++;
  },
);
...
if (process.env.ALCYONE_LIVE_SUBSCRIPTION_URL) {
  ...
  subscriptions.download(
    process.env.ALCYONE_LIVE_SUBSCRIPTION_URL,
    function (liveError, liveResult) {
      if (liveError) {
        console.log(
          "# optional live subscription check skipped: " + liveError.code,
        );
        return;
      }
      console.log(
        "# optional live subscription check: " +
          liveResult.imported.length +
          " servers",
      );
    },
  );
...
process.on("exit", function (code) {
  if (code === 0 && subscriptionChecks !== 2) {
    console.error(
      "subscription fixture checks did not both run (" +
        subscriptionChecks +
        "/2)",
    );
    process.exitCode = 1;
...
var tunServiceExePath =
  "usr/palm/services/com.alcyone.vpn.service/bin/tun2socks";
...
assert.ok(
  !xrayEntries[xrayServiceExePath],
  "XRay service payload must not duplicate bin/xray",
);
assert.ok(
  !xrayEntries[tunServiceExePath],
  "XRay service payload must not duplicate bin/tun2socks",
);
assert.strictEqual(
  xrayEntries[xrayExePath].mode & 0o111,
  0o111,
  "bin/xray must have 0755 executable permission",
);
assert.strictEqual(
  xrayEntries[tunExePath].mode & 0o111,
  0o111,
  "bin/tun2socks must have 0755 executable permission",
);

/* The routing databases are large too and must likewise appear exactly once. */
assert.ok(
  xrayEntries["usr/palm/applications/com.alcyone.vpn/bin/geoip.dat"],
  "geoip.dat must ship with the app payload",
);
assert.ok(
  !xrayEntries["usr/palm/services/com.alcyone.vpn.service/bin/geoip.dat"],
  "service payload must not duplicate geoip.dat",
);
assert.ok(
  !xrayEntries["usr/palm/services/com.alcyone.vpn.service/bin/geosite.dat"],
  "service payload must not duplicate geosite.dat",
);

/* Verify ARM ELF Header for XRay binaries */
assert.strictEqual(
  xrayEntries[xrayExePath].data.toString("ascii", 1, 4),
  "ELF",
  "xray must be an ELF binary",
);
assert.strictEqual(
  xrayEntries[xrayExePath].data.readUInt16LE(18),
  40,
  "xray binary must target ARM (e_machine == 40)",
);

console.log(
  "ok 2 - XRay IPK carries ARM ELF cores once, in the app payload, with 0755 permissions",
);

/* Verify sing-box IPK contents */
var singboxExePath =
  "usr/palm/applications/com.alcyone.vpn.singbox/bin/sing-box";
var singboxServiceExePath =
  "usr/palm/services/com.alcyone.vpn.singbox.service/bin/sing-box";

assert.ok(
  singboxEntries[singboxExePath],
  "sing-box IPK app must contain bin/sing-box",
);
assert.ok(
  !singboxEntries[singboxServiceExePath],
  "sing-box service payload must not duplicate bin/sing-box",
);
assert.strictEqual(
  singboxEntries[singboxExePath].mode & 0o111,
  0o111,
  "bin/sing-box must have 0755 executable permission",
);
assert.strictEqual(
  singboxEntries[singboxExePath].data.toString("ascii", 1, 4),
  "ELF",
  "sing-box must be an ELF binary",
);
assert.strictEqual(
  singboxEntries[singboxExePath].data.readUInt16LE(18),
  40,
  "sing-box binary must target ARM (e_machine == 40)",
);

console.log(
  "ok 3 - sing-box IPK carries its ARM ELF core once, in the app payload, with 0755 permission",
);
...
function assertNoDuplicateLargeEntries(entries, label) {
  ...
  Object.keys(byBasename).forEach(function (base) {
    assert.strictEqual(
      byBasename[base],
      1,
      label + " duplicates large payload " + base,
    );
  });
}
...
assert.strictEqual(
  resolvedXrayCores.tun2socks,
  "/private/alcyone/bin/tun2socks",
);
assert.deepStrictEqual(prepared, [
  {
    packaged: "/packaged/alcyone/bin/xray",
    staged: "/private/alcyone/bin/xray",
    name: "xray",
  },
  {
    packaged: "/packaged/alcyone/bin/tun2socks",
    staged: "/private/alcyone/bin/tun2socks",
    name: "tun2socks",
  },
]);

console.log(
  "ok 4 - VpnManager verifies packaged cores into fixed staged executable paths",
);
