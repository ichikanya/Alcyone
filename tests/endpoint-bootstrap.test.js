...
record(
  "hostname validation accepts ordinary and LAN-local DNS names",
  bootstrap.isValidHostname("edge.example.com") &&
    bootstrap.isValidHostname("vpn.lan") &&
    bootstrap.isValidHostname("vpn"),
);
record(
  "hostname validation rejects controls, underscores and malformed labels",
...
record(
  "literal detection distinguishes valid IPv4/IPv6 from colon garbage",
...
asyncTest(
  "public system DNS endpoint resolves before bootstrap",
  function (done) {
    runResolve(
      [{ host: "edge.example.com", port: 443, network: "tcp" }],
      {
        "edge.example.com": [{ address: "93.184.216.34", family: 4 }],
      },
      function (error, result, calls) {
        record(
          "domain endpoint with public system DNS succeeds",
...
asyncTest("LAN-local DNS answers are valid proxy endpoints", function (done) {
  runResolve(
    [{ host: "vpn.lan", port: 8443 }],
    {
      "vpn.lan": [{ address: "192.168.50.20", family: 4 }],
    },
    function (error, result) {
      record(
        "domain endpoint with LAN-local DNS succeeds",
...
asyncTest("literal IPv4 bypasses DNS", function (done) {
  runResolve(
    ...
    function (error, result, calls) {
      record(
        "literal IPv4 endpoint performs no DNS",
        !error && calls.length === 0 && result.addresses[0] === "198.51.100.8",
      );
      record(
        "literal IPv4 does not create static hostname configuration",
        !result.hasMappings(),
      );
      done();
    },
...
asyncTest("answers and duplicate endpoints are deduplicated", function (done) {
  runResolve(
    ...
    function (error, result, calls) {
      record(
        "multiple resolved IPv4 candidates are retained",
        !error && result.addresses.join(",") === "93.184.216.34,93.184.216.35",
      );
      record(
        "duplicate endpoint hostnames share one lookup and one mapping",
        calls.length === 1 &&
...
asyncTest("failed DNS is a bounded structured error", function (done) {
  runResolve(
    [{ host: "missing.example", port: 443 }],
    {
      "missing.example": new Error("resolver failed"),
    },
    function (error, result) {
      record(
        "failed resolution returns ENDPOINT_RESOLUTION_FAILED",
...
asyncTest(
  "an AAAA-only answer is rejected without leaking it",
  function (done) {
    runResolve(
      [{ host: "v6.example", port: 443 }],
      {
        "v6.example": [{ address: "2001:db8::10", family: 6 }],
      },
      function (error) {
        record(
          "no IPv4 result returns ENDPOINT_RESOLUTION_FAILED",
          !!error &&
            error.code === "ENDPOINT_RESOLUTION_FAILED" &&
...
asyncTest("invalid hostname is rejected before DNS", function (done) {
  runResolve(
    [{ host: "bad_name.example", port: 443 }],
    {},
    function (error, result, calls) {
      record(
        "invalid hostname never reaches the resolver",
        !!error &&
          error.code === "ENDPOINT_RESOLUTION_FAILED" &&
          !result &&
          calls.length === 0,
...
asyncTest(
  "IPv6 literal is detected but rejected before DNS on an IPv4 tunnel",
  function (done) {
    runResolve(
      [{ host: "2001:db8::10", port: 443 }],
      {},
      function (error, result, calls) {
        record(
          "IPv6 literal performs no DNS and fails before route takeover",
          !!error &&
            error.code === "ENDPOINT_RESOLUTION_FAILED" &&
            !result &&
            calls.length === 0,
...
asyncTest("resolver callback is exactly once", function (done) {
  ...
  runResolve(
    ...
    function () {
      ...
      setTimeout(function () {
        record(
          "bootstrap callback fires exactly once",
          callbackCount === 1,
...
asyncTest("hung resolver is bounded", function (done) {
  var callbackCount = 0;
  bootstrap.resolve(
    [{ host: "timeout.example", port: 443 }],
    function (error) {
      callbackCount++;
      setTimeout(function () {
        record(
          "resolution timeout is bounded and exactly once",
          !!error &&
            error.code === "ENDPOINT_RESOLUTION_FAILED" &&
            callbackCount === 1,
        );
...
record(
  "generated XRay config uses the pinned exact full-host key form",
  generatedXray.dns.hosts["edge.example.com"][0] === "93.184.216.34" &&
    !generatedXray.dns.hosts["full:edge.example.com"],
);
record(
  "generated XRay matching outbound uses sockopt UseIP",
  generatedOutbound.streamSettings.sockopt.domainStrategy === "UseIP",
);
record(
  "generated XRay outbound domain remains unchanged",
  generatedOutbound.settings.vnext[0].address === "Edge.Example.com",
);
record(
  "generated XRay preserves SNI, REALITY and WebSocket Host",
  generatedOutbound.streamSettings.realitySettings.serverName ===
    "sni.example.com" &&
    generatedOutbound.streamSettings.realitySettings.publicKey === "PUBLIC" &&
    generatedOutbound.streamSettings.wsSettings.headers.Host ===
      "cdn.example.com" &&
  ...
  outbounds: [
    {
      ...
      settings: {
        vnext: [
          { address: "EDGE.EXAMPLE.COM", port: 443, users: [{ id: "SECRET" }] },
        ],
      },
      streamSettings: {
        network: "grpc",
    ...
    },
    {
      ...
      settings: {
        servers: [
          { address: "edge.example.com", port: 8443, password: "SECRET" },
        ],
      },
      streamSettings: {
        network: "ws",
...
record(
  "full imported XRay config gets the exact bootstrap mapping",
  fullBuilt.dns.hosts["edge.example.com"][0] === "93.184.216.34",
);
record(
  "compatible explicit XRay domain strategy is preserved",
...
record(
  "XRay TLS, SNI, REALITY, gRPC and WebSocket fields remain unchanged",
  fullBuilt.outbounds[0].streamSettings.realitySettings.serverName ===
    "reality.example" &&
    fullBuilt.outbounds[0].streamSettings.grpcSettings.authority ===
      "authority.example" &&
    fullBuilt.outbounds[1].streamSettings.tlsSettings.allowInsecure === false &&
    fullBuilt.outbounds[1].streamSettings.wsSettings.headers.Host ===
      "host.example",
);
record(
  "building a full XRay config never rewrites the stored profile object",
  JSON.stringify(fullSource) === sourceBefore,
);

var fullEndpoints = xray.endpoints({ fullConfig: fullSource });
record(
  "full XRay endpoint extraction keeps duplicate hosts on distinct ports",
  fullEndpoints.length === 2 &&
    fullEndpoints[0].port === 443 &&
    fullEndpoints[1].port === 8443,
);

var generatedSingbox = singbox.build(generatedProfile, pin);
record(
  "sing-box uses the pinned 1.13 hosts-server schema",
  generatedSingbox.dns.servers[0].type === "hosts" &&
    generatedSingbox.dns.servers[0].tag === singbox.BOOTSTRAP_TAG &&
    generatedSingbox.dns.servers[0].predefined["edge.example.com"][0] ===
      "93.184.216.34",
);
record(
  "sing-box matching outbound points its domain resolver at static hosts",
  generatedSingbox.outbounds[0].domain_resolver.server ===
    singbox.BOOTSTRAP_TAG &&
    generatedSingbox.outbounds[0].domain_resolver.strategy === "ipv4_only",
);
record(
  "sing-box outbound domain and TLS/WebSocket semantics are preserved",
...
var sbCustom = {
  dns: {
    servers: [
      { type: "udp", tag: singbox.BOOTSTRAP_TAG, server: "192.0.2.53" },
    ],
  },
...
record(
  "sing-box preserves user DNS and chooses a collision-free bootstrap tag",
  sbCustom.dns.servers.length === 2 &&
    sbCustom.dns.servers[1].type === "udp" &&
    sbCustom.outbounds[0].domain_resolver.server ===
      singbox.BOOTSTRAP_TAG + "-2",
);
...
function managerHarness(options) {
  ...
  var manager = new vpnLib.VpnManager({
    ...
    logger: {
      info: function (message, data) {
        logText += message + JSON.stringify(data || {});
      },
    ...
    lock: {
      ...
      status: function () {
        return { held: false, mine: false };
      },
    },
    coreIntegrity: {
      prepare: function (packaged, staged) {
        return staged;
      },
      verifyForLaunch: function (file) {
        return file;
      },
    },
  ...
  manager.resolveEndpoints = function (profile, callback) {
    ...
    if (options.resolveFails) {
      return process.nextTick(function () {
        callback(
          errors.err("ENDPOINT_RESOLUTION_FAILED", "endpoint lookup failed"),
        );
      });
    }
    process.nextTick(function () {
      callback(null, currentBootstrap);
    });
  };
  manager.routes.saveState = function (addresses) {
    ...
    this.state = {
      ...
      serverAddresses: addresses.slice(0),
    };
    return this.state;
  };
  manager.routes.loadState = function () {
    return this.state;
  };
  manager.routes.applyTunRoutes = function () {
    sequence.push("routes");
    routeCalls.applied++;
  };
  manager.routes.rollback = function () {
    routeCalls.rolledBack++;
    this.state = null;
  };
  manager.routes.routeActive = function () {
    return true;
  };
  ...
  manager.routes.diagnostics = function () {
    return {};
  };
  manager.writeConfig = function () {
  ...
  manager.coresAlive = function () {
    return true;
  };
  manager.supervisor.stopAll = function (callback) {
    process.nextTick(callback);
  };
  manager.supervisor.status = function () {
    return {};
  };
  ...
  return {
    ...
    setBootstrap: function (value) {
      currentBootstrap = value;
    },
...
asyncTest("resolution precedes all startup state changes", function (done) {
  ...
  ctx.manager.connect(function (error) {
    record(
      "resolution occurs before lock, snapshot, core and route changes",
      !error &&
        ctx.sequence.join(",") ===
    ...
    record(
      "the same resolved addresses are saved for bypass routes",
      ctx.manager.routes.state.serverAddresses[0] === "203.0.113.10",
    );
    record(
      "manager logs never expose endpoint hostname or address",
...
asyncTest(
  "failed resolution leaves routes and lock untouched",
  function (done) {
    var ctx = managerHarness({ resolveFails: true });
    ctx.manager.connect(function (error) {
      record(
        "failed resolution leaves network state untouched",
        !!error &&
          error.code === "ENDPOINT_RESOLUTION_FAILED" &&
          ctx.routeCalls.saved === 0 &&
          ctx.routeCalls.applied === 0 &&
          ctx.routeCalls.rolledBack === 0 &&
          ctx.lockCalls.acquired === 0 &&
  ...
  ctx.manager.connect(function (firstError) {
    ...
    ctx.manager.disconnect(function (disconnectError) {
      ...
      ctx.manager.connect(function (secondError) {
        record(
          "server switching performs a fresh resolution and bypass snapshot",
          !secondError &&
            ctx.sequence.filter(function (item) {
              return item === "resolve";
            }).length === 2 &&
