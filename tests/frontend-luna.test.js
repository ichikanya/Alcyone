...
record('frontend never references Homebrew Channel exec or spawn',
...
var hbMethods = [];
var hbCallRe = /lunaAt\(\s*([A-Za-z_$][\w$]*)\s*,\s*'([^']+)'/g;
var hbMatch;
while ((hbMatch = hbCallRe.exec(source)) !== null) {
  if (hbMatch[1].indexOf('HBCHANNEL') >= 0) hbMethods.push(hbMatch[2]);
}
record('frontend calls only allow-listed Homebrew Channel methods',
  hbMethods.every(function (name) { return HB_ALLOWED_METHODS.indexOf(name) >= 0; }),
...
record('frontend still performs the allow-listed elevation call',
...
var hbVariableMethodRe = /lunaAt\(\s*[A-Za-z_$][\w$]*HBCHANNEL[\w$]*\s*,\s*[^'\s]/;
record('Homebrew Channel method names are string literals',
...
record('Homebrew Channel URI is a literal constant',
  source.indexOf("'luna://org.webosbrew.hbchannel.service'") >= 0 ||
...
record('Homebrew Channel URI appears exactly once', hbUriOccurrences === 1, String(hbUriOccurrences));
record('Homebrew Channel URI is never concatenated',
...
record('elevation target ids are a fixed closed list',
  /ELEVATABLE_SERVICE_IDS\s*=\s*\[\s*'com\.alcyone\.vpn\.service'\s*,\s*'com\.alcyone\.vpn\.singbox\.service'\s*\]/.test(source));
record('elevation id is validated against that list before use',
  /ELEVATABLE_SERVICE_IDS\.indexOf\(id\)\s*>=\s*0/.test(source));

/* No user-controlled source may flow into an elevation call. */
var elevateCall = source.slice(source.indexOf("lunaAt(HBCHANNEL, 'elevateService'"));
elevateCall = elevateCall.slice(0, elevateCall.indexOf('\n') + 1);
record('elevation call passes only the validated constant id',
  /lunaAt\(HBCHANNEL, 'elevateService', \{ id: id \}, ELEVATE_TIMEOUT_MS,/.test(elevateCall),
  elevateCall.trim());
['value', 'search', 'profile', 'subscription', 'input', 'params.'].forEach(function (token) {
  record('elevation call carries no ' + token, elevateCall.indexOf(token) < 0);
});
record('frontend has no shell quoting helper', source.indexOf('shQuote') < 0);
record('frontend does not build core configs',
  source.indexOf('buildConfig') < 0 && source.indexOf('streamSettings') < 0);
record('frontend does not parse proxy links', source.indexOf('parseProxyLink') < 0);
record('frontend has no direct HTTP client', source.indexOf('XMLHttpRequest') < 0);
record('frontend references the retired control script nowhere', source.indexOf('alcyonectl') < 0);
record('frontend does not reference the unrelated legacy app', source.indexOf('vless.m.vpn') < 0);

/* --- runtime behaviour -------------------------------------------------- */

var PROFILES = [
  {
    id: 'p1', name: 'NL Reality', protocol: 'vless', country: 'nl', endpoint: 'a.example.com:443',
    transport: 'ws', security: 'reality', sourceType: 'single', subscriptionId: '', subscriptionName: '',
    hasFullConfig: false, selected: true
  },
  {
    id: 'p2', name: 'DE Node', protocol: 'trojan', country: 'de', endpoint: 'b.example.com:443',
    transport: 'tcp', security: 'tls', sourceType: 'subscription', subscriptionId: 's1',
    subscriptionName: 'My Sub', hasFullConfig: false, selected: false
  }
];
...
['stateText', 'hint', 'power', 'current', 'homeStage', 'serverList', 'count', 'search',
 'pingServers', 'refresh', 'subUpdate', 'log', 'logsRefresh', 'clearLog', 'freezeLog',
 ...
 'aboutVersion', 'webUrl', 'webState', 'webSub'].forEach(function (id) {
...
function serviceRespond(method, params, onSuccess) {
  if (method === 'getProfiles') {
    return onSuccess({
      returnValue: true, ok: true, profiles: PROFILES,
      subscriptions: [{ id: 's1', name: 'My Sub', host: 'panel.example.com', count: 1, lastUpdate: 0, hasError: false }],
      activeId: 'p1', lang: 'auto', revision: 'r1'
    });
  }
  ...
  if (method === 'getState') {
    return onSuccess({
      ...
      edition: { id: 'xray', core: 'xray', coreLabel: 'XRay', title: 'Alcyone XRay', version: '3.2.1' },
...
var windowStub = {
  ALCYONE_EDITION: {
    appId: 'com.alcyone.vpn', serviceId: 'com.alcyone.vpn.service', core: 'xray',
    coreLabel: 'XRay', editionName: 'XRay Edition', title: 'Alcyone XRay', version: '3.2.1'
  },
  ...
  webOS: {
    service: {
      request: function (uri, options) {
        calls.push({ uri: uri, method: options.method, parameters: options.parameters });
        serviceRespond(options.method, options.parameters, options.onSuccess || function () {});
      }
    }
  }
};
...
var methods = calls.map(function (c) { return c.method; });
record('frontend calls only the Alcyone service URI',
  calls.every(function (c) {
    return c.uri === 'luna://com.alcyone.vpn.service' || c.uri === 'luna://com.webos.settingsservice';
  }),
  calls.map(function (c) { return c.uri; }).join(','));
record('frontend requests state on boot', methods.indexOf('getState') >= 0);
record('frontend requests profiles on boot', methods.indexOf('getProfiles') >= 0);
record('no call carries a command parameter',
  calls.every(function (c) { return !c.parameters || c.parameters.command === undefined; }));

record('server list renders from sanitized metadata',
  elements.serverList.innerHTML.indexOf('NL Reality') >= 0 &&
  elements.serverList.innerHTML.indexOf('DE Node') >= 0);
record('server list shows endpoint and protocol badges',
  elements.serverList.innerHTML.indexOf('a.example.com:443') >= 0 &&
  elements.serverList.innerHTML.indexOf('VLESS') >= 0);
record('server list groups subscription profiles',
  elements.serverList.innerHTML.indexOf('My Sub') >= 0);
record('home shows the selected server', elements.current.innerHTML.indexOf('NL Reality') >= 0);
record('profile count is localized', /2\s/.test(elements.count.textContent), elements.count.textContent);

/* Power button must ask the service to connect, not run anything. */
calls.length = 0;
elements.power.onclick();
var connectCall = calls.filter(function (c) { return c.method === 'connect'; });
record('power button issues a connect method', connectCall.length === 1);
record('connect call sends a structured payload',
...
record('pairing start calls startPairing',
  calls.filter(function (c) { return c.method === 'startPairing'; }).length === 1);
record('pairing code is displayed on the TV',
  elements.pairBox.innerHTML.indexOf('ABCD2345') >= 0);
record('pairing shows the LAN address',
...
record('log viewer requests bounded service logs',
  calls.filter(function (c) { return c.method === 'getLogs'; }).length === 1);
record('log viewer renders both service and VPN core output',
...
calls.length = 0;
elements.rowAutostart.onclick();
record('autostart toggle calls setAutostart',
  calls.filter(function (c) { return c.method === 'setAutostart'; }).length === 1);
