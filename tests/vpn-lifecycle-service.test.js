...
var lockLib = require(
  path.join(ROOT, "app", "service", "lib", "tunnel-lock.js"),
);
...
function makeManager(options) {
  ...
  store.upsertManualProfile(
    "vless://11111111-2222-3333-4444-555555555555@a.example.com:443?security=tls&type=ws#Node",
    "Node",
  );
  ...
  manager.routes.networkChanged = function () {
    return !!options.networkChanged;
  };
  manager.routes.routeActive = function () {
    return !options.routeInactive;
  };
  ...
  manager.resolveCores = function () {
    return {
      xray: "/fake/xray",
      tun2socks: "/fake/tun2socks",
      singbox: "/fake/sing-box",
    };
  };
  ...
  manager.supervisor.isRunning = function (name) {
    return !!running[name];
  };
  manager.supervisor.stopAll = function (cb) {
    running = {};
    manager._tunUp = false;
    setImmediate(cb);
  };
...
step(function (next) {
  ...
  ctx.manager.connect(function (error) {
    record(
      "successful connect reaches connected state",
      !error && ctx.manager.state === "connected",
      error ? error.code : "",
    );
    record("successful connect installs routes", ctx.events.applied === 1);
    record(
      "startup retains its validated route snapshot instead of rereading it",
      ctx.events.loaded === 0,
    );
    record(
      "successful connect verifies real traffic before reporting connected",
      ctx.events.probed === 1 &&
        ctx.manager.status().dataPlaneVerified === true,
    );
    record(
      "successful connect holds the tunnel lock",
      ctx.manager.lock.status().mine === true,
    );
    record(
      "XRay uses the bundled legacy tun2socks CLI",
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        ...
        record(
          "physical network change is detected while connected",
          !error && ctx.manager.checkNetworkChange(),
        );
        setTimeout(function () {
          record(
            "network change reports a stable error and returns to idle",
            ctx.manager.state === "idle" &&
              ctx.manager.lastError &&
              ctx.manager.lastError.code === "NETWORK_CHANGED",
          );
          record(
            "network change rolls back the tunnel exactly once",
            ctx.events.rolledBack === before + 1,
          );
          next();
        }, 30);
      });
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        ...
        record(
          "healthy route guard leaves the connected tunnel intact",
          ctx.manager.state === "connected" && ctx.events.rolledBack === before,
        );
        ctx.manager.disconnect(function () { next(); });
      });
    });
  })
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        ctx.manager.routes.networkChanged = function () { return false; };
        ctx.manager.routes.routeActive = function () { return true; };
        ctx.manager.routes.directRoutesActive = function () { return false; };
        record(
          "lost direct bypass is treated as a route fault",
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        record(
          "core spawn failure returns an error",
          !!error,
          error ? error.code : "none",
        );
        record(
          "core spawn failure rolls routes back",
          ctx.events.rolledBack >= 1,
        );
        record(
          "core spawn failure releases the lock",
          ctx.manager.lock.status().held === false,
        );
        record(
          "core spawn failure returns to idle",
          ctx.manager.state === "idle",
        );
        next();
      });
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        record(
          "tun never appearing fails cleanly",
          !!error,
          error ? error.code : "none",
        );
        record("tun failure rolls routes back", ctx.events.rolledBack >= 1);
        record(
          "tun failure releases the lock",
          ctx.manager.lock.status().held === false,
        );
        next();
      });
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        record(
          "route install failure is reported",
          !!error,
          error ? error.code : "none",
        );
        record("route install failure rolls back", ctx.events.rolledBack >= 1);
        record(
          "route install failure stops children",
          Object.keys(ctx.running()).length === 0,
        );
        next();
      });
    });
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        record(
          "inactive tunnel route fails the health check",
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function (error) {
        record(
          "failed external traffic verification is reported accurately",
          !!error && error.code === "HEALTH_CHECK_FAILED",
          error ? error.code : "none",
        );
        record(
          "traffic verification failure never reports connected",
          ctx.manager.state === "idle" &&
            ctx.manager.status().connected === false,
        );
        record(
          "traffic verification failure rolls routes back and stops children",
  ...
  .then(function () {
    return step(function (next) {
      ...
      ctx.manager.connect(function () {
        ...
        setTimeout(function () {
          record(
            "core crash while connected rolls routes back",
            ctx.events.rolledBack > before,
          );
          record(
            "core crash releases the lock",
            ctx.manager.lock.status().held === false,
          );
  ...
  .then(function () {
    return step(function (next) {
      ...
      var xrayLock = new lockLib.TunnelLock({
        edition: "xray",
        lockFile: lockFile,
      });
      var singboxLock = new lockLib.TunnelLock({
        edition: "sing-box",
        lockFile: lockFile,
      });
      xrayLock.acquire();
      ...
      record(
        "second edition cannot take the tunnel",
        blocked && code === "TUNNEL_OWNED_BY_OTHER_EDITION",
        code,
      );
      record(
        "other edition cannot release our lock",
        singboxLock.release() === false,
      );
      xrayLock.release();
