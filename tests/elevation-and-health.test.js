...
function armElf(extra) {
  var size = 20 + (extra || 0);
  var buffer =
    typeof Buffer.alloc === "function" ? Buffer.alloc(size) : new Buffer(size);
...
check(
  "ARMv7 cores remain valid on an ARM64 Linux compatibility userspace",
  function () {
    assert.strictEqual(
      healthLib.machineMatchesRuntime(40, "linux", "arm64"),
      true,
    );
    assert.strictEqual(
      healthLib.machineMatchesRuntime(62, "linux", "arm64"),
      false,
    );
  },
);
...
check(
  "Homebrew root plus a jailed Alcyone service reports ELEVATION_REQUIRED",
  function () {
    var problem = gate().check({
      homebrewRoot: true,
      privilege: { root: false },
    });
...
check(
  "a jailed service never reports CORE_MISSING for an intact package",
  function () {
    var problem = gate().check({
      homebrewRoot: true,
      privilege: { root: false },
...
check(
  "a jailed service still reports ELEVATION_REQUIRED when cores are absent",
  function () {
    ...
    var problem = gate({
      paths: { appDir: bare, dataDir: path.join(bare, "nope") },
...
check(
  "an elevated service with a genuinely missing core still reports CORE_MISSING",
  function () {
...
check(
  "a missing package payload reports PACKAGE_INCOMPLETE before CORE_MISSING",
  function () {
    var problem = gate({
      paths: { appDir: path.join(tmp, "absent"), dataDir: dataDir },
    }).check({ homebrewRoot: true, privilege: { root: true } });
...
check(
  "a non-ELF core reports CORE_INTEGRITY_FAILED, not CORE_MISSING",
  function () {
...
check("an empty core reports CORE_INTEGRITY_FAILED", function () {
  ...
  var problem = gate({
    paths: { appDir: empty, dataDir: path.join(empty, "nope") },
  }).check({ homebrewRoot: true, privilege: { root: true } });
  assert.strictEqual(codeOf(problem), "CORE_INTEGRITY_FAILED");
});
...
check("a corrupt routing asset reports ASSET_INTEGRITY_FAILED", function () {
  ...
  var problem = gate({
    paths: { appDir: assetDir, dataDir: path.join(assetDir, "nope") },
  }).check({ homebrewRoot: true, privilege: { root: true } });
...
check("the sing-box edition requires no XRay routing assets", function () {
  ...
  var problem = gate({
    edition: { core: "sing-box" },
    paths: { appDir: sb, dataDir: path.join(sb, "nope") },
  }).check({ homebrewRoot: true, privilege: { root: true } });
  assert.strictEqual(codeOf(problem), "OK");
});
...
check("the documented gate order is exactly the implemented one", function () {
  ...
  for (i = 0; i < order.length; i++)
    assert.ok(errors.CODES[order[i]], "missing code " + order[i]);
});
...
check("repeated polling does not re-hash routing assets", function () {
  ...
  xrayAssets.sha256File = function (file) {
    hashed++;
    return realSha(file);
  };
  try {
...
check(
  "the integrity cache invalidates when file metadata changes",
  function () {
    ...
    assert.notStrictEqual(
      g.integritySignature(),
      first,
      "signature must track size",
    );
  },
);
...
check("a permission denial is reported as denied, not absent", function () {
  var inspection = supervisorLib.isExecutableFile(
    path.join(appDir, "bin", "xray"),
  );
  assert.strictEqual(
    typeof inspection,
    "object",
  ...
  assert.ok(
    "exists" in inspection &&
      "executable" in inspection &&
      "reason" in inspection,
  );
});

check("a genuinely absent file reports ENOENT, never EACCES", function () {
  var inspection = supervisorLib.isExecutableFile(
    path.join(tmp, "no-such-binary"),
  );
  assert.strictEqual(inspection.exists, false);
  assert.strictEqual(
    supervisorLib.isPermissionDenied(inspection),
    false,
...
check("isPermissionDenied recognises EACCES and EPERM only", function () {
  assert.strictEqual(
    supervisorLib.isPermissionDenied({ reason: "EACCES" }),
    true,
  );
  assert.strictEqual(
    supervisorLib.isPermissionDenied({ reason: "EPERM" }),
    true,
  );
  assert.strictEqual(
    supervisorLib.isPermissionDenied({ reason: "ENOENT" }),
    false,
  );
...
check(
  "a denied core lookup is classified as a jail, not a missing binary",
  function () {
    ...
    g.locateCore = function () {
      return { file: "", denied: true };
    };
    try {
...
check("resolveFailureReason separates denial from absence", function () {
  assert.strictEqual(
    supervisorLib.resolveFailureReason([path.join(tmp, "nothing-here")]),
    "ENOENT",
  );
  assert.strictEqual(
    supervisorLib.resolveFailureReason([path.join(appDir, "bin", "xray")]),
    "",
  );
});
...
check("privilege exposes no filesystem paths", function () {
  ...
  assert.ok(
    serialized.indexOf(tmp) < 0,
    "privilege leaked a path: " + serialized,
  );
  assert.ok(
    serialized.indexOf("/var/lib") < 0,
    "privilege leaked a data directory",
  );
  assert.ok(
    Object.keys(facts).length === 6,
    "unexpected extra privilege fields: " + serialized,
  );
});
...
check("no Phase 0 probe marker survives anywhere in the tree", function () {
  ...
  (function walk(dir) {
    fs.readdirSync(dir).forEach(function (entry) {
      ...
      if (
        text.indexOf("PHASE 0 PROBE") >= 0 ||
        text.indexOf("PHASE0_PROBE") >= 0 ||
        text.indexOf("probeElevationState") >= 0 ||
        text.indexOf("probeLog") >= 0 ||
...
check("probe-phase0.js is gone", function () {
  assert.strictEqual(
    fs.existsSync(path.join(ROOT, "app", "service", "lib", "probe-phase0.js")),
    false,
  );
});
...
check("the builder no longer offers a probe mode", function () {
  ...
  assert.ok(
    builder.indexOf("PROBE") < 0,
    "a PROBE marker survives in the builder",
  );
});
...
check("the Luna method count is exactly 26 after subscription HWID support", function () {
  /* 21 original methods plus restartService, setAutostartProfile, setDnsServer, setConnectionMode.
     Counted from the source rather than by loading service.js, which would start a service. */
  ...
  assert.strictEqual(names.length, 26, "METHODS entries: " + names.length);
});

check("services.json declares exactly the same 26 commands", function () {
  var declared = JSON.parse(
    fs.readFileSync(path.join(ROOT, "app", "service", "services.json"), "utf8"),
  );
  var commands = declared.services[0].commands;
  assert.strictEqual(
    commands.length,
    26,
    "declared commands: " + commands.length,
  );
  assert.ok(
    commands.some(function (c) {
      return c.name === "restartService";
    }),
    "restartService must survive",
  );
  /* probeProfiles is a production method (it measures server latency) and must
     survive; only the two Phase 0 commands are gone. */
  assert.ok(
    commands.some(function (c) {
      return c.name === "probeProfiles";
    }),
    "probeProfiles must survive",
  );
...
check(
  "restartService remains declared, registered and implemented",
  function () {
    ...
    var api = fs.readFileSync(
      path.join(ROOT, "app", "service", "lib", "api.js"),
      "utf8",
    );
    assert.ok(/restartService:\s*["']restartService["']/.test(service));
...
check("requestRestart is idempotent", function () {
  ...
  assert.ok(/restartPending\s*\|\||if \(restartPending\) return true;/.test(block));
});

check(
  "restartService never spawns, kills by name or runs a shell",
  function () {
    var service = fs.readFileSync(
      path.join(ROOT, "app", "service", "service.js"),
      "utf8",
...
check("neither frontend nor service edits LS2 configuration", function () {
  ...
  [
    ...
    assert.ok(
      app.indexOf(token) < 0,
      "frontend references LS2 config: " + token,
    );
  });
});
...
check("the frontend has no shell access of any kind", function () {
  var app = fs.readFileSync(path.join(ROOT, "app", "app.js"), "utf8");
  ["/exec", "/spawn", "child_process", "shQuote", "execSync"].forEach(
    function (token) {
...
check("the service never calls Homebrew Channel itself", function () {
  ...
  (function walk(dir) {
    fs.readdirSync(dir).forEach(function (entry) {
      ...
      if (fs.readFileSync(full, "utf8").indexOf("hbchannel") >= 0)
...
check("the elevation banner exposes no internals to the screen", function () {
  var app = fs.readFileSync(path.join(ROOT, "app", "app.js"), "utf8");
  var start = app.indexOf("function renderElevation");
  if (start < 0) start = app.indexOf("function It(");
  var block = app.slice(start);
  ...
  /* Only localized keys reach the DOM here — never a bus payload, a path or a
     stringified error. */
  assert.ok(
    block.indexOf("JSON.stringify") < 0,
    "banner stringifies a payload",
  );
  assert.ok(block.indexOf("errorText") < 0 && block.indexOf(".detail") < 0);
  assert.ok(
    /(?:tr|ge)\(["']hb\.title["']\)/.test(block) &&
      /(?:tr|ge)\(["']elev\.title["']\)/.test(block),
  );
});
...
check(
  "the builder ships cores under the application payload only",
  function () {
    ...
    [
      ...
      assert.ok(
        block.indexOf(token) < 0,
        "service payload duplicates " + token,
      );
    });
    assert.ok(
      builder.indexOf(
